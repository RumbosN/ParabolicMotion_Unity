public static class Constants
{
    public static string LeftControllerAnchor = "LeftControllerAnchor";
    public static string RightControllerAnchor = "RightControllerAnchor";
    public static float VelocityOffset = 10.5f;
    public static float Gravity = -10.0f;
    public static int KeyboardInteractiveLayer = 13;
    public static float MinDistanceHandToActiveAza = 1.0f;
    public static int TimeToSpawnSnowBall = 5;
    public static string GoalTag = "Goal";
    public static string StorageDataFile = "ParabolicBattleData.json";
}
